...
#データ（メンバー辞書）
df = pd.read_excel("SAKURA_list.xlsx")

allmenber=dict(zip(df["Name"],df["Player_No"]))
allmenber_list=[f"No,{s} {t}" for t ,s in allmenber.items()]
...
if st.session_state.get("confirmed", False):
    ...
    if st.button("OK"):
        ...
        try:
            ...
            df[today]=" "
            #選ばれた人だけ "Selected" 
            for name in selected_menber.keys():
                df.loc[df["Name"]==name,today]="Serected"
            for name in staff_list:
                df.loc[df["Name"]==name,today]="Staff"
            #保存
            df.to_excel("SAKURA_list.xlsx",index=False)
            st.success("Comp")
